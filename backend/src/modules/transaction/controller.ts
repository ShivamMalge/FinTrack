...
export const getOne = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.id;
    const tx = await getTransactionById(userId, req.params.id as string);
    if (!tx) {
...
export const update = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.id;
    const tx = await updateTransaction(userId, req.params.id as string, req.body);
    res.status(200).json(successResponse(tx));
  } catch (error: any) {
...
export const remove = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.id;
    await deleteTransaction(userId, req.params.id as string);
